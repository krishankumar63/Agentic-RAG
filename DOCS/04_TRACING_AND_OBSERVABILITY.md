...
Logfire records the agentic flow and runtime details for the project, including:
...
> [!TIP]
> All traces are linked via a common `trace_id`. If you find a bug in the UI, you can inspect the exact LLM call and the corresponding Logfire span using that ID.

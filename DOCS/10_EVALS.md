...
A second, separate LLM that scores the output of your RAG system. Instead of hardcoded rules, you ask an LLM: _"Does this answer faithfully use the context? Score 0 to 1."_
...
Whether the answer is _supported by_ the source context. A grounded answer only uses information present in the retrieved chunks. An ungrounded answer adds information from the LLM's training data or makes things up.
...
| **Logfire**          | Runtime tracing and observability                         | Custom + built-in                                       | ✅            | ⚠️ Good but generic | ✅               |
...
| Full LangChain pipeline tracing                  | **Logfire**              |
